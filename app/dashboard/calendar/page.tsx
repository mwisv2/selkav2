...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    if (profile) {
      ...
      // Add a workout for today if there isn't one already
      const todayWorkout = generatedWorkouts.find((workout) => weekdays[today.getDay()] === workout.day)

      if (todayWorkout) {
        events.push({
          date: new Date(today),
          type: todayWorkout.title,
      ...
      generatedWorkouts.forEach((workout: any) => {
        ...
        // Skip if we already added this day as today
        if (workoutDay === today.getDay()) return

        const daysToAdd = (workoutDay + 7 - today.getDay()) % 7
        if (daysToAdd === 0) return // Skip if it's today (already added)
        ...
        // Add next week's workouts too
        const nextWeekDate = new Date(workoutDate)
        nextWeekDate.setDate(workoutDate.getDate() + 7)
