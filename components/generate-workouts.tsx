...
const filterExercisesByEquipment = (exercises: Exercise[], equipment: string[]): Exercise[] => {
  ...
  const bodyweightExercises = exercises.filter(ex => 
    Array.isArray(ex.equipment) && ex.equipment.includes("No Equipment (Bodyweight only)")
  );
...
const selectExercisesForBodyParts = (
  bodyParts: string[],
  availableExercises: Exercise[],
  count: number
): string[] => {
  // First, try to get bodyweight exercises for the target body parts
  const bodyweightExercises = availableExercises.filter(ex => 
    Array.isArray(ex.equipment) && 
    ex.equipment.includes("No Equipment (Bodyweight only)") &&
    bodyParts.includes(ex.bodyPart)
  );

  // Then get equipment-based exercises
  const equipmentExercises = availableExercises.filter(ex => 
    Array.isArray(ex.equipment) && 
    !ex.equipment.includes("No Equipment (Bodyweight only)") &&
    bodyParts.includes(ex.bodyPart)
  );

  // Combine both sets and select randomly
  const allExercises = [...bodyweightExercises, ...equipmentExercises];
  return shuffleArray(allExercises).slice(0, count).map(ex => ex.name);
};
