...
export default function BenchPressPage() {
  ...
  const handleWeightChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value) {
      ...
      const normalized = Math.min(Math.max(numValue / (unit === "kg" ? 200 : 440), 0.3), 1)
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-5 duration-500">
      <Card className="border-none shadow-lg">
        <CardHeader>
          <CardTitle className="text-2xl">Bench Press Max</CardTitle>
          <CardDescription>What's your one-rep maximum for bench press?</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <Tabs defaultValue="kg" onValueChange={handleUnitChange} className="w-full">
              ...
              <div className="w-full px-2 py-4">
                ...
                <div className="flex justify-between text-sm text-muted-foreground mt-2">
                  <span>0 {unit}</span>
                  <span>{unit === "kg" ? "100 kg" : "220 lb"}</span>
                  <span>{unit === "kg" ? "200 kg" : "440 lb"}</span>
                </div>
              </div>

              <div className="space-y-2 mt-4">
                <Label htmlFor="bench-press">Bench Press Max ({unit})</Label>
                <Input
                  id="bench-press"
                  type="number"
            ...
            <div className="flex gap-3">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push("/onboarding/workout-split")}
                className="flex-1"
              >
