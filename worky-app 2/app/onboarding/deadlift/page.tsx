...
export default function DeadliftPage() {
  ...
  const handleWeightChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value) {
      ...
      const normalized = Math.min(Math.max(numValue / (unit === "kg" ? 300 : 660), 0.3), 1)
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-5 duration-500">
      <Card className="border-none shadow-lg">
        <CardHeader>
          <CardTitle className="text-2xl">Deadlift Max</CardTitle>
          <CardDescription>What's your one-rep maximum for deadlift?</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <Tabs defaultValue="kg" onValueChange={handleUnitChange} className="w-full">
              ...
              <div className="w-full px-2 py-4">
                ...
                <div className="flex justify-between text-sm text-muted-foreground mt-2">
                  <span>0 {unit}</span>
                  <span>{unit === "kg" ? "150 kg" : "330 lb"}</span>
                  <span>{unit === "kg" ? "300 kg" : "660 lb"}</span>
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="deadlift">Deadlift Max ({unit})</Label>
                <Input
                  id="deadlift"
                  type="number"
                  placeholder={unit === "kg" ? "140" : "310"}
                  value={weight}
            ...
            <div className="flex gap-3">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push("/onboarding/squat")}
                className="flex-1"
              >
